<template>
	<view>
		...
		<view class="cu-card case isCard">
			<view class="cu-item shadow">
				...
				<view class="cu-list padding">
					...
					<view class="cu-item">充币数:{{orderinfo.totalamount}} {{orderinfo.unit}}(请一定按此数量充币)</view>
					<view class="cu-item">
						<view class="content flex-sub">
							<view class="text-grey">钱包地址：{{orderinfo.walletaddress}}</view>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="flex flex-direction padding" v-show="orderinfo.status!=1">
			<button class="cu-btn bg-blue margin-tb-sm lg" @tap="dodel()">取消本次充币</button>
		</view>
	  ...
	  dodel(){
        this.app.get({
            ...
            success: res => {
					...
					this.app.navTo('/pages/assets/details',{coinid:this.orderinfo.cointype});
            },
            errmsg: res => {}
          });
