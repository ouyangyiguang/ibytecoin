    ...
    <view class="flexView">
      <view class="scrollView">
        <view class="head-read">
          <view class="flex">
            ...
            <view class="flex-box">
              ...
              <view class="flex-box-text">{{ coininfo.namecode }}</view>
            </view>

            <view class="tx-btn" v-if="coininfo.id == 1"
              ><span class="tx-grid-comm-sign">充值</span></view
            >
            <view
              class="tx-btn"
              v-if="coininfo.id != 1"
              @click="
                $tools.routerTo(
                  '/pages/my/assets/congcoin',
                  'selectcoinid=' + coininfo.id
                )
              "
              ><span class="tx-grid-comm-sign">充币</span></view
            >
            <view
              class="tx-btn"
              v-if="coininfo.id != 1"
              @click="
                $tools.routerTo(
                  '/pages/my/assets/ticoin',
                  'selectcoinid=' + coininfo.id
                )
              "
              ><span class="tx-grid-comm-sign">立即提币</span></view
            >
          </view>
        </view>
        <view class="white-box">
          <view class="tx">
            <view class="tx-grid">
              <view><span class="tx-grid-comm-sign">总额</span></view>
            </view>
            <view class="tx-grid">
              <view class="tx-grid-text">
                <view class="money">
                  {{ (coininfo.balance_total * 1).toFixed(4) }}
                  <text class="money-b">{{ coininfo.unit }}</text>
                </view>
              </view>
            </view>
          </view>
              ...
              <view class="palace-grid-text">
                <view class="palace-grid-text-data">
                  ...
                  <text class="palace-grid-text-data-b"
                    >{{ (coininfo.balance_available * 1).toFixed(4) }}
                    {{ coininfo.unit }}</text
                  >
                </view>
                <view class="palace-grid-text-name" @click="showModal = true"
                  >可用余额</view
                >
              </view>
            </view>
            <view class="palace-grid">
              <view class="palace-grid-text">
                <view class="palace-grid-text-data">
                  <text></text>
                  <text class="palace-grid-text-data-b"
                    >{{ (coininfo.balance_forzen * 1).toFixed(4) }}
                    {{ coininfo.unit }}</text
                  >
                </view>
                <view class="palace-grid-text-name" @click="showModal = true"
          ...
          >
            <view class="content-box">
              ...
              <view class="des-box" v-else>
                <view class="tit">
                  {{
                    currentTab == 3 ? "流水号：" + item.id : "SN：" + item.sn
                  }}
                  <u-tag
                    :text="item.behaviorname"
                    :type="item.opera == '+' ? 'success' : 'error'"
                    size="mini"
                    class="u-m-l-10"
                  />
                </view>
            ...
            <view
              class="money"
              v-if="currentTab != 3"
              :class="item.opera == '+' ? 'jia' : 'shao'"
              >{{ currentTab == 3 ? item.coin : (item.coin * 1).toFixed(4) }}
              {{ coininfo.unit }}</view
            >
          </view>
    ...
    <u-popup v-model="showModal" mode="center">
      <view class="cu-modal">
        <view class="cu-dialog">
          ...
          <view
            class=""
            style="text-align: left; padding: 0rpx 35rpx 50rpx 35rpx"
          >
            <view>
            ...
            <view class="u-m-t-10"
              ><text class="u-type-info-dark"
                >*注：所有金额币种均为{{ coinname }}，单位为{{
                  coininfo.unit
                }}</text
              ></view
            >
